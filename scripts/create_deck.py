...
# Only bump when fields change. Template/CSS changes are safe with same ID.
...
/* Scale Anki's native replay button on the Listening card front.
   .replay-button is generated by Anki from [sound:file.mp3].
   Without this, it's a tiny icon. We scale to 80px so it's the
   dominant visual element on the otherwise-empty front. */
...
/* Pitch accent colors from generate_pitch_accent.py.
   Each mora in the PitchAccent field is wrapped in pitch-h or pitch-l spans.
   Green = high pitch, Red = low pitch (Tokyo dialect). */
